...
public class twothree {
   ...
   static void printValuesInRange(TwoThreeTree tree, String startKey, String endKey){
	   
	   if(startKey.compareTo(endKey) <= 0){

		   printValuesInRange(tree.root, startKey, endKey, tree.height);
	   } 
	   else {
		   printValuesInRange(tree.root, endKey, startKey, tree.height);
	   }
   }
	
   private static void printValuesInRange(Node node, String startKey, String endKey, int height){
	   if(height == 0){
		   if(node.guide.compareTo(startKey) >= 0 && node.guide.compareTo(endKey) <= 0){
			   System.out.println(node.guide + " " + ((LeafNode) node).value);
		   }
	   ...
	   if(iN.child0.guide.compareTo(startKey) >= 0){
		   printValuesInRange(iN.child0, startKey, endKey, height - 1);
	   } 
	   
	   if(iN.child1.guide.compareTo(startKey) >= 0){
		   printValuesInRange(iN.child1, startKey, endKey, height - 1);
	   }
	   
	   if(iN.child2 != null && iN.child2.guide.compareTo(startKey) >= 0){
		   printValuesInRange(iN.child2, startKey, endKey, height - 1);
	   } 
   }
	
   static int getValue(Node node, String key, int height){
	   ...
	   if(iN.child0.guide.compareTo(key) >= 0){
		   return getValue(iN.child0, key, height - 1);
	   }else if(iN.child1.guide.compareTo(key) >= 0){
		   return getValue(iN.child1, key, height - 1);
	   }else if(iN.child2 != null){
		   return getValue(iN.child2, key, height - 1);
	   } else {
		   return -1;
	   }
   }
